          ...
          child: pw.Column(
            ...
            children: [
              ...
              if (biz.address != null && biz.address!.isNotEmpty)
                pw.Text(
                  biz.address!,
                  style: const pw.TextStyle(color: PdfColors.white, fontSize: 8.5, height: 1.25),
                ),
    ...
    children: [
      pw.TableRow(
        ...
        children: [
          ...
          _cell(_money.format(l.landingCost), align: pw.TextAlign.right, fontSize: 8.5),
          _cell(_money.format(amt), align: pw.TextAlign.right, fontSize: 8.5),
        ],
      ),
    ],
  );
  ...
  return pw.Column(
    ...
    children: [
      pw.Table(
        ...
        children: [
          moneyRow('Subtotal (items)', _money.format(sub)),
          if (disc > 0) moneyRow('Discount (%)', '- ${_money.format(sub * disc / 100)}'),
          if (freight > 0) moneyRow('Freight', '+ ${_money.format(freight)}'),
          if (commAmt > 0) moneyRow('Commission (${commPct.toStringAsFixed(1)}%)', '+ ${_money.format(commAmt)}'),
          if (billty > 0) moneyRow('Billty / charges', '+ ${_money.format(billty)}'),
          if (delivered > 0) moneyRow('Delivered rate', '+ ${_money.format(delivered)}'),
          moneyRow('GRAND TOTAL', '₹ ${_money.format(p.totalAmount)}', bold: true),
        ],
      ),
      ...
      pw.Row(
        ...
        children: [
          pw.Text('Paid: ₹ ${_money.format(p.paidAmount)}', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
          pw.Text('Balance: ₹ ${_money.format(p.remaining)}', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
          if (p.dueDate != null)
