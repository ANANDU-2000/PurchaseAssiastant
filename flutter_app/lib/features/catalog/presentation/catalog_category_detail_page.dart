...
class CatalogCategoryDetailPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CatalogCategoryDetailPage> createState() => _CatalogCategoryDetailPageState();
}

class _CatalogCategoryDetailPageState extends ConsumerState<CatalogCategoryDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final itemsInCat = itemsAsync.maybeWhen(
      data: (items) => items.where((it) => it['category_id']?.toString() == widget.categoryId).toList(),
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
                ...
                return Card(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('Category performance', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
                        const SizedBox(height: 12),
                        _row(context, 'Catalog items', '${ins['item_count']}'),
                        _row(context, 'Purchase lines', '${ins['linked_line_count']}'),
                        _row(context, 'Total profit', _inr(_num(ins['total_profit']))),
                        if (ins['top_item_name'] != null)
                          _row(context, 'Top item', '${ins['top_item_name']} (${_inr(_num(ins['top_item_profit']))})'),
                        if (ins['worst_item_name'] != null)
                          _row(context, 'Weakest item', '${ins['worst_item_name']} (${_inr(_num(ins['worst_item_profit']))})'),
                      ],
                    ),
                  ),
                );
            ...
            Text('Items in category', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
  ...
  Widget _row(BuildContext context, String a, String b) {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(width: 120, child: Text(a, style: Theme.of(context).textTheme.bodySmall)),
          Expanded(child: Text(b, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600))),
        ],
      ),
    );
  }
