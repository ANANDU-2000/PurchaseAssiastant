...
class _CatalogItemDetailPageState extends ConsumerState<CatalogItemDetailPage> {
  ...
  Future<void> _addVariant() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(controller: ctrl, decoration: const InputDecoration(labelText: 'Name (e.g. 1L, 5L)')),
            const SizedBox(height: 8),
            TextField(
              controller: kg,
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
              decoration: const InputDecoration(labelText: 'Default kg/bag (optional)'),
            ),
          ],
        ),
        actions: [
    ...
    try {
      ...
      ref.invalidate(catalogVariantsProvider(widget.itemId));
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Variant added')));
    } on DioException catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${e.response?.data ?? e}')));
      }
    }
  }
  ...
  Future<void> _editVariant(Map<String, dynamic> v) async {
    ...
    final kg = TextEditingController(
      text: v['default_kg_per_bag'] != null ? v['default_kg_per_bag'].toString() : '',
    );
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
    ...
    final session = ref.read(sessionProvider);
    if (session == null) return;
    final kgVal = kg.text.trim().isEmpty ? null : double.tryParse(kg.text.trim());
    try {
  ...
  Widget build(BuildContext context) {
    ...
    String? catName;
    if (catsAsync.hasValue && itemAsync.hasValue) {
      final cid = itemAsync.value!['category_id']?.toString();
    ...
    return Scaffold(
      appBar: AppBar(
        title: itemAsync.when(
          data: (m) => Text(m['name']?.toString() ?? 'Item', maxLines: 1, overflow: TextOverflow.ellipsis),
        ...
        child: ListView(
          ...
          children: [
            if (catName != null && catName.isNotEmpty)
              Text(
                catName,
                style: Theme.of(context).textTheme.labelLarge?.copyWith(color: HexaColors.textSecondary),
              ),
            Text(
                ...
                return Card(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('Performance', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
                        const SizedBox(height: 12),
                        Wrap(
                          spacing: 12,
                          runSpacing: 8,
                          children: [
                            ...
                            if (pm != null) _chip(context, 'Margin %', '${(pm as num).toStringAsFixed(1)}%'),
                            if (ld != null) _chip(context, 'Last purchase', ld),
                          ],
                        ),
                      ],
                    ),
            ...
            Text('Variants', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
            const SizedBox(height: 8),
            varsAsync.when(
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            IconButton(icon: const Icon(Icons.edit_outlined), onPressed: () => _editVariant(v)),
                            IconButton(icon: const Icon(Icons.delete_outline), onPressed: () => _deleteVariant(v)),
                          ],
                        ),
                      ),
                    );
            ...
            Text('Purchase history', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
            const SizedBox(height: 8),
            linesAsync.when(
