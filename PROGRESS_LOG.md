...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-14 | Flutter web: defer `apiDegradedProvider` updates (`session_notifier` Dio callback, `main` warmup, degraded banner Retry/dismiss) via `addPostFrameCallback` to avoid `markNeedsBuild during build` when interceptors run mid-frame. |
